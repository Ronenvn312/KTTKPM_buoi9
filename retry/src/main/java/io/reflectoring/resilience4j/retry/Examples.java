...
public class Examples {
    ...
    void generalUsagePattern() {
        ...
        Supplier<List<Flight>> flightSearchSupplier
                = () -> service.searchFlights(request); // ----> 4
    ...
    void predicateExample() {
        RetryConfig config =
                RetryConfig
                        .<SearchResponse>custom()
                        .maxAttempts(3)
                        .waitDuration(Duration.of(3, SECONDS))
                        .retryOnResult(
                                searchResponse ->
                                        searchResponse.getErrorCode()
                                                .equals("FS-167"))
                        .build();
        ...
        SearchRequest request =
                new SearchRequest("NYC", "LAX", "07/31/2020");
        CheckedFunction0<SearchResponse> retryingFlightSearch
                = Retry.decorateCheckedSupplier(retry,
                () -> service.httpSearchFlights(request));
    ...
    void retryExceptionsIgnoreExceptionsExample() {
        ...
        System.out.println("Example to illustrate: rate limit runtime exception -" +
                " will be retried");
        ...
        Supplier<BookingResponse> bookingResponseSupplier
                = () -> service.bookFlight(request);
        Supplier<BookingResponse> bookingResponse
                = Retry.decorateSupplier(retry, bookingResponseSupplier);
    ...
    void asyncRetryExample()  {
        ...
        Supplier<CompletionStage<List<Flight>>>
                completionStageSupplier =
                () -> CompletableFuture.supplyAsync(
                        () -> service.searchFlights(request));

        retry.executeCompletionStage(scheduler,
                completionStageSupplier).thenAccept(System.out::println);
    ...
    void retryEventsExample() {
        RetryConfig config
                ...
                .custom()
                .maxAttempts(3)
                .waitDuration(Duration.of(1, SECONDS)).build();
        ...
        SearchRequest request = new SearchRequest("NYC", "LAX", "07/31/2020");
        List<Flight> flights =
                retry.executeSupplier(() -> service.searchFlights(request));
        System.out.println(flights);
    }

    void retryOnException() {
        Predicate<Throwable> rateLimitPredicate
                = rle -> (rle instanceof  RateLimitExceededException)
                   && "RL-101".equals(((RateLimitExceededException) rle)
                .getErrorCode());
